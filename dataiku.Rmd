...
Normally for building a model I'd want to dig deeper into a few of the encoded categories, but for the purposes of this mini-assignment I'm just going to treat them as blind categories.
...
model <- glm(income.group ~ model.class.of.worker.private + 
...
model.learn$predict <- predict(model, type='response')
...
roctest <- roc(factor(income.group) ~ predict, data=model.learn)
...
# 
# #model.rf <- randomForest(income.group ~ ., data = learn)
